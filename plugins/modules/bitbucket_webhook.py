...
version_added: "1.0.0"
description:
  - Create, update, and delete bitbucket webhook resources.
...
options:
  state:
    description:
      - Desired state of the bitbucket webhook resource.
    type: str
...
def get_current_state(client, module):
    """Retrieve the current state of the bitbucket webhook via GET."""
        ...
        if state == "present":
            ...
            if current is not None:
                ...
                if not module.check_mode:
                    ...
                    identifier = current.get("id")
                    path = "/repositories/{workspace}/{repo_slug}/hooks/{uid}".replace(
                        "{id}", str(identifier)
                    )
                    client.delete(path)
